// app/api/users/Time-sheet-approval/send/route.js
...
export async function POST(req) {
  try {
    ...
    console.log("user_id received in Time-sheet-approval/send:", user_id);
    const uid = Number(user_id);
    if (!Number.isFinite(uid) || uid <= 0) {
      return NextResponse.json({ message: "Valid user_id is required" }, { status: 400 });
    }

    const value = await usersService.setTimeSheetApproval(uid, 1);
    return NextResponse.json({ time_sheet_approval: value, message: "Sent (1)" });
  } catch (e) {
