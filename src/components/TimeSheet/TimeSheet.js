  ...
  function buildDailyPayablesForUser(detailsByDate, userId) {
    ...
    const out = [];
    for (const [date, sessions] of Object.entries(detailsByDate)) {
      // only this user's sessions, not already processed (flagger !== 1)
      const mine = (sessions || []).filter(
        (s) => Number(s?.user_id) === Number(userId) && Number(s?.flagger) !== 1
      );
      if (!mine.length) continue;

      const totalSecs = mine.reduce((acc, s) => acc + (s?.seconds || 0), 0);
      const hours = Math.round((totalSecs / 3600) * 100) / 100;
      // if you want purely “hours > 0” days only:
      if (!(hours > 0)) continue;
      ...
      out.push({
        id: `${userId}-${date}`, // any unique id for the UI submitter
        date, // used by submitter to mark processed
        hours,
        payment,
        serial_ids,
        // for your own visibility:
        _debug: { count: mine.length, totalSecs, payment },
      });
    }
...
// useEffect(() => {
//   // Check if the selectedUser is not 'all' and localDetailsByDate data is available
//   if (selectedUser !== "all" && localDetailsByDate) {
//     // Get all sessions for the selected user from localDetailsByDate
//     const selectedUserSessions = Object.values(localDetailsByDate)
//       .flat() // Flatten the sessions into a single array
//       .filter((session) => session.user_id === selectedUser); // Filter sessions for the selected user

//     // Log all session data with the date
//     selectedUserSessions.forEach((session) => {
//       const date = format(new Date(session.startISO), "yyyy-MM-dd"); // Format the date from startISO
//       console.log(`Date: ${date}, Serial ID: ${session.serial_id}, ` +
//         `Task ID: ${session.task_id}, Project: ${session.project_name}, ` +
//         `Hours: ${session.seconds / 3600}, Payment: ${session.session_payment}, ` +
//         `User: ${session.user_name}`);
//     });

//     // Optionally log the full filtered sessions for the selected user
//     console.log(`Filtered sessions for ${selectedUserName}:`, selectedUserSessions);
//   }
// }, [selectedUser, localDetailsByDate, selectedUserName]);
...
useEffect(() => {
  ...
  if (selectedUser !== "all" && localDetailsByDate) {
    ...
    const selectedUserSessions = Object.values(localDetailsByDate)
      .flat() // Flatten the sessions into a single array
      .filter((session) => session.user_id === selectedUser && session.flagger === 0); // Filter sessions for the selected user with flagger = 0

    // Initialize an object to group sessions by date
    const groupedByDate = {};

    selectedUserSessions.forEach((session) => {
      const date = format(new Date(session.startISO), "yyyy-MM-dd"); // Format the date from startISO
      const hours = (session.seconds / 3600).toFixed(2); // Calculate hours from seconds

      // If the date is not already in the grouped object, add it
      if (!groupedByDate[date]) {
        groupedByDate[date] = {
          date,
          hours: 0,
          label: "", // Label is a string to represent the total hours for the day
          serial_ids: [], // List to store serial IDs for the day
        };
      }

      // Add session data to the grouped object for the current date
      groupedByDate[date].hours += parseFloat(hours); // Add hours for the day
      groupedByDate[date].serial_ids.push(session.serial_id); // Add serial ID for the day
      groupedByDate[date].label = `${groupedByDate[date].hours}h`; // Update the label with total hours
    });

    // Convert the groupedByDate object into an array
    const groupedSessions = Object.values(groupedByDate);

    // Log the grouped sessions
    console.log("Grouped Sessions (flagger = 0):", groupedSessions);
  }
}, [selectedUser, localDetailsByDate]); // Only use selectedUser and localDetailsByDate as dependencies
  ...
  return buildFilteredSeries(
    ...
    useEffect(() => {
      ...
      try {
        ...
        console.log("Daily payables (unprocessed) to be submitted on Approve:", dailyPayablesForSelected);
    ...
    async function handleAdminApproveSelected() { //.....oke admin er id(full currentUser) pathaite hbe............
      if (!isSelectedFreelancer) return;
      try {
        setAdminSelLoading(true);

        // 1) Your existing approval API
        ...
        // 2) Use the locally built payables (exactly what we logged)
        ...
        // 3) Ensure we pass a usable currentUser for txn factory
        const currentUserForTxn = {
          ...(currentUser || {}),
          // we keep the admin object, but if id/tenant are missing we fallback so the flow can continue for tests
          id: currentUser?.id ?? selectedUser, // fallback: attribute txn developer_id to freelancer if admin id missing
          tenant_id: currentUser?.tenant_id ?? "9999", // obvious fallback for debug
        };

        if (!currentUser?.tenant_id || !currentUser?.id) {
          console.warn(
            "⚠️ Using fallback currentUserForTxn to continue:",
            currentUserForTxn
          );
        }
        ...
        // 4) Submit with the same shared action used by Payroll
        ...
        //   // If your PaymentCommonApi supports developerId override, keep this:
